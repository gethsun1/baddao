    ...
    <Section className="py-12 md:py-20 animate-in fade-in duration-700 min-h-screen">
      ...
      {isLoading && (
        <div className="flex flex-col items-center justify-center py-32 space-y-4">
          <Loader2 className="animate-spin text-baddao-accent" size={48} />
          <p className="text-gray-400 font-mono text-sm">Querying indexer...</p>
        </div>
      )}

      {!isLoading && (!daos || daos.length === 0) && (
        <div className="glass p-16 md:p-24 rounded-3xl border border-baddao-border/50 text-center space-y-6">
          <div className="w-20 h-20 bg-baddao-primary rounded-2xl flex items-center justify-center mx-auto border border-baddao-border">
            <Shield className="w-10 h-10 text-gray-500" />
          </div>
          <div>
            <h3 className="text-2xl font-bold text-white mb-2">No organizations found</h3>
            <p className="text-gray-400 max-w-md mx-auto">There are currently no active DAOs deployed on this network through BAD DAO Factory.</p>
          </div>
