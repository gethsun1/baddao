...
export function DaoCard({ name, address, creator, createdAt, onClick }: DaoCardProps) {
  return (
    <Card 
      onClick={onClick}
      className="p-6 cursor-pointer group hover:border-communityCorner-accent/40 hover:shadow-[0_0_20px_rgba(47,107,255,0.05)] transition-all"
    >
      <div className="flex items-start justify-between mb-6">
        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-communityCorner-accent/20 to-blue-600/20 border border-communityCorner-accent/20 flex items-center justify-center">
          <Shield size={24} className="text-communityCorner-highlight group-hover:text-white transition-colors" />
        </div>
      ...
      <div className="border-t border-communityCorner-border/50 pt-4 flex justify-between text-sm items-end">
